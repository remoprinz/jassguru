...
function calculateChartData(allSessionsSnap, calculateDelta, getTournamentDelta, tournamentRankings, isEventChart = false) {
  ...
  allSessionsSnap.docs.forEach(doc => {
    ...
    allPlayerNames.forEach((_, playerId) => {
      ...
      // 🎯 KORREKTUR: Prüfe Tournament-Rankings NUR wenn diese für DIESE Session vorhanden sind
      ...
      // ✅ Fallback: Wenn kein Tournament-Delta gefunden wurde, verwende normale Berechnung
      if (delta === null) {
        if (isTopPlayer) {
