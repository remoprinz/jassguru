...
const createInitialStateLocal = (initialPlayer: PlayerNumber): GameState => ({
  ...
  scoreSettings: {
    values: {berg: 0, sieg: 0, schneider: 0},
    enabled: {berg: true, sieg: true, schneider: true},
    isFlipped: false,
  },
...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  startGame: (players: GamePlayers, initialStartPlayer: PlayerNumber): void => {
    ...
    set((state): Partial<GameState> => {
      ...
      return {
        gamePlayers: players,
        playerNames: names,
        currentPlayer: initialStartPlayer,
        startingPlayer: initialStartPlayer,
  ...
  navigateHistory: (direction: NavigationDirection) => {
    set((state) => {
      ...
      if (newIndex === -1) {
        const initialState = {
          ...createInitialStateLocal(state.startingPlayer),
  ...
  resetGameState: (playerNumber?: PlayerNumber) => {
    console.warn("resetGameState not fully implemented");
    set(createInitialStateLocal(playerNumber || 1));
  },

  rebuildStateFromHistory: (index: number) => {
    console.warn("rebuildStateFromHistory not implemented");
    // Implementierungslogik fehlt
  },
  ...
  setFarbeSettings: (settings: FarbeSettings) => {
    console.warn("setFarbeSettings not fully implemented");
    set({farbeSettings: settings});
  },
}));
