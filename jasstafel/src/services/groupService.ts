    ...
    for (const groupDoc of groupDocs) {
      ...
      for (const pId of groupPlayerIds) {
        ...
        if (currentPlayerData) {
          const displayName = currentPlayerData.nickname || currentPlayerData.displayName || "Unbekannt";
  ...
  try {
    ...
    for (const groupDoc of groupDocs) {
      ...
      for (const pId of groupPlayerIds) {
        ...
        if (currentPlayerData) {
          // Verwende den aktuellen Nickname oder DisplayName
          const displayName = currentPlayerData.nickname || currentPlayerData.displayName || "Unbekannt";
          // Versuche, joinedAt aus den Originaldaten zu übernehmen, sonst Fallback
          const joinedAt = originalPlayerData?.joinedAt instanceof Timestamp
            ? originalPlayerData.joinedAt
            : Timestamp.now(); // Fallback auf aktuelle Zeit
    ...
    // Gib die angereicherten Gruppen zurück
