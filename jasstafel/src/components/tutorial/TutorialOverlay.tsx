...
const TutorialOverlay: React.FC<TutorialOverlayProps> = ({onCloseMenu}) => {
  ...
  const handleEndTutorial = useCallback(() => {
    ...
    if (currentStep?.category) {
      markCategoryAsCompleted(currentStep.category);
    }

    // 4. Tutorial beenden, aber hasCompletedTutorial nur setzen wenn Checkbox aktiv
    endTutorial(neverShowAgain);
  }, [currentStep?.category, endTutorial, markCategoryAsCompleted, neverShowAgain]);
