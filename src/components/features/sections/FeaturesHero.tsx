...
/* ── Jass-Karten (identisch wie Home HeroSection) ─────────── */
...
const FeaturesHero: React.FC<FeaturesHeroProps> = ({ onCta }) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        <Image src="/images/backgrounds/felt-figma.webp" alt="" fill className="object-cover" priority quality={75} />
      </div>
      ...
      >
        <Image src="/images/backgrounds/holztisch.webp" alt="" fill priority className="object-cover" aria-hidden="true" />
        <div className="absolute inset-0 z-[1] pointer-events-none" style={{ borderRadius: 'inherit', boxShadow: 'inset 0 4px 16px rgba(0,0,0,0.4), inset 0 -2px 8px rgba(0,0,0,0.2), inset 4px 0 12px rgba(0,0,0,0.15), inset -4px 0 12px rgba(0,0,0,0.15)' }} />
        <div className="absolute z-[2] overflow-hidden" style={{ inset: 'clamp(18px, 3.5vw, 44px)', borderRadius: 'clamp(4px, 0.8vw, 10px)' }}>
          <Image src="/images/backgrounds/chalkboard-jvs.webp" alt="" fill priority className="object-cover" />
          <div className="absolute inset-0 z-10 pointer-events-none" style={{ borderRadius: 'inherit', boxShadow: 'inset 0 3px 12px rgba(0,0,0,0.6), inset 0 -2px 6px rgba(0,0,0,0.3), inset 3px 0 8px rgba(0,0,0,0.2), inset -3px 0 8px rgba(0,0,0,0.2)' }} />
        </div>
      </div>
      ...
      {cards?.desktop.map((card, i) => (
        ...
        >
          <Image src={card.src} alt="Jasskarte" width={159} height={250} className="w-full h-auto" style={{ borderRadius: '14px', filter: 'drop-shadow(0 15px 25px rgba(0,0,0,0.4))' }} />
        </motion.div>
      ))}
      ...
      {cards?.mobile.map((card, i) => (
        ...
        >
          <Image src={card.src} alt="Jasskarte" width={159} height={250} className="w-full h-auto" style={{ borderRadius: '6px', filter: 'drop-shadow(0 8px 16px rgba(0,0,0,0.4))' }} />
        </motion.div>
      ))}
      ...
      >
        ...
        >
          ...
          >
            Jasstafel, Kalkulator, Rangliste, Elo-Rating und Turniere — alles in einer App.
          </p>
        </motion.div>
      ...
      >
        <img src="/images/icons/pfeil_unten_weiss.svg" alt="" width={24} height={28} className="opacity-80" aria-hidden="true" />
      </motion.div>
    </section>
  );
};
