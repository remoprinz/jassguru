...
const RoundInfo: React.FC<RoundInfoProps> = ({
  ...
  const fadeProps = useSpring({
    ...
    config: {duration: 150},
    delay: startFadeIn && !isOpen ? 350 : 0,
    immediate: isOpen,
  });
  ...
  const formatTeamNames = (isTopTeam: boolean) => {
    ...
    // Bestimme den aktiven Spieler für die Hervorhebung
    const activePlayer = !isGameStarted ? startingPlayer : currentPlayer;

    return (
      <div className="flex items-center gap-1.5">
        <span className={activePlayer === player1 ?
          "text-white font-bold text-xl" :
          "text-gray-500"}>
          {name1}
        </span>
        <span className="text-gray-500">+</span>
        <span className={activePlayer === player2 ?
          "text-white font-bold text-xl" :
          "text-gray-500"}>
          {name2}
        </span>
