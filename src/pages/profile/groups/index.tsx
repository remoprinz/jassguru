...
const GroupsPage: React.FC = () => {
  ...
  // Funktion für Einladungsverarbeitung
  const handleProcessInviteInput = useCallback(async (inputValue: string) => {
    if (!showNotification || !router) {
        console.error("showNotification oder router nicht initialisiert in handleProcessInviteInput (GroupsPage)");
        return;
    }
    setIsProcessingInvite(true);

    const result = extractAndValidateToken(inputValue, "group");

    if (result.token) {
      if (result.type === 'group') {
        console.log(`[GroupsPage] Navigiere zu /join mit Gruppentoken: ${result.token}`);
        router.push(`/join?token=${result.token}`);
      } else {
        showNotification({
          message: result.error || "Ungültiger Gruppencode.",
          type: "error",
        });
      }
      ...
      <MainLayout>
        <div className="flex min-h-screen flex-col items-center bg-gray-900 p-4 text-white relative">
          ...
          <div className="w-full max-w-md space-y-6 py-16"> {/* Added padding top */}
            ...
            {/* Neue Komponente: Einladungsfunktion für Gruppen */}
            <JoinByInviteUI 
              inviteType="group" 
