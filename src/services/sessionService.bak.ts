...
/**
 * Entfernt die Referenz auf das aktive Spiel (`currentActiveGameId`)
 * aus einem Jass-Session-Summary-Dokument in Firestore.
 *
 * @param sessionId Die ID der Jass-Session, deren aktives Spiel entfernt werden soll.
 * @returns Ein Promise, das aufgelöst wird, wenn das Update erfolgreich war.
 */
export const clearActiveGameInSession = async (sessionId: string): Promise<void> => {
  ...
  try {
    ...
    console.log(`[clearActiveGameInSession] Updating session ${sessionId} to remove currentActiveGameId.`);
    await updateDoc(sessionDocRef, {
      currentActiveGameId: deleteField(),
    ...
    console.error(`[clearActiveGameInSession] Error removing active game from session ${sessionId}:`, error);
    throw error;
  }
};
