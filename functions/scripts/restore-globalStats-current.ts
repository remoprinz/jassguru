...
interface JassGameSummary {
  ...
  gamesPlayed: number;
  durationSeconds?: number; // ✅ NEU: Spielzeit pro Session
  teams?: {
    ...
    for (const playerId of playerIdsToProcess) {
      try {
        ...
        const globalStats: any = {
          ...
          sessionsLost: 0,
          sessionsDraw: 0,
          totalTournaments: 0, // ✅ NEU: Turniere zählen
          totalGames: 0,
          gamesWon: 0,
          gamesLost: 0,
          trumpfStatistik: {},
          totalTrumpfCount: 0,
          totalPlayTimeSeconds: 0 // ✅ NEU: Gesamtspielzeit akkumulieren
        };
        ...
        for (const summary of summaries) {
          ...
          // ✅ SPIELZEIT (durationSeconds aus jassGameSummary)
          if (summary.durationSeconds && typeof summary.durationSeconds === 'number') {
            globalStats.totalPlayTimeSeconds += summary.durationSeconds;
          }
        }
        ...
        if (!isDryRun) {
          ...
          console.log(`      - Spielzeit: ${Math.round(globalStats.totalPlayTimeSeconds / 60)} Minuten`);
