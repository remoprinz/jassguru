...
async function main() {
  ...
  if (args.length === 0) {
    ...
    await createCorrectedPointsChartDataForAllGroups();
...
async function createCorrectedPointsChartDataForAllGroups(): Promise<void> {
  console.log(`[correctedPointsBackfill] Starting corrected points chart backfill for all groups`);
  
  const groupsSnapshot = await db.collection('groups').get();
  const groups = groupsSnapshot.docs.map(doc => doc.id);
  
  console.log(`[correctedPointsBackfill] Found ${groups.length} groups to process`);
  ...
  for (const groupId of groups) {
    try {
      ...
      console.error(`[correctedPointsBackfill] Error processing group ${groupId}: ${error}`);
      skipCount++;
    }
  }
  
  console.log(`[correctedPointsBackfill] 🎉 Backfill completed! Success: ${successCount}, Skipped: ${skipCount}`);
}

export { createCorrectedPointsChartData, createCorrectedPointsChartDataForAllGroups };
